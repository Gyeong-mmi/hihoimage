...
public class Manager_ResultInDetail : CLASS_XmlData
{
    ...
    public void Add_RIDdata(int idx, float data_1, float data_2 = -999, float data_3 = -999)   //idx+데이터 3가지
    {
        string Data_merged;

        if (idx < 0) return;

        Data_merged = idx.ToString() + ",";
        ...
        {
            Data_merged += data_1.ToString() + "," + data_2.ToString() + "," + data_3.ToString();
        }
        //데이터 리스트에 추가
