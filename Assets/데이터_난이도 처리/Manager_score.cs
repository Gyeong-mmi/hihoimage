...
public class Manager_score : CLASS_XmlData
{
    ...
    void Start()
    {
        ...
        if (filePath != null)
        {
            ...
            // RESULT_score1 파일에 필터링된 데이터 쓰기
            WriteToXml(LastDataList1);
        }
    }
    ...
    {
        // RESULT_A 파일 경로 설정
        string filePathA = Path.Combine(Application.persistentDataPath, "RESULT_score1.xml");

        // XML 파일 생성
        XmlDocument document = new XmlDocument();
        XmlElement itemListElement = document.CreateElement("Result_score1");
        document.AppendChild(itemListElement);

        // 데이터 리스트를 순회하면서 XML에 추가
        foreach (Result_IndetailData data in dataList)
        {
            XmlElement itemElement = document.CreateElement("Result_score1");
            itemElement.SetAttribute("ID", data.ID);
            itemElement.SetAttribute("Name", data.Name);
            itemElement.SetAttribute("Date", data.Date);
            itemElement.SetAttribute("Session", data.Session);
